...
class lite_b2b_model extends CI_Model
{
    ...
    public function get_customer($check_super_admin, $user_guid)
    {
        ...
        if ($check_super_admin > 0) {
            $query = $this->db->query("SELECT acc_guid,UPPER(acc_name) as acc_name,seq,
              IF(acc_guid = '13EE932D98EB11EAB05B000D3AA2838A', 'http://b2b.xbridge.my/asset/uploads/pandamart/pandamart.jpg' , CONCAT('http://b2b.xbridge.my/asset',file_path,'/',SUBSTRING_INDEX(file_path, '/', -1) , '.jpg') ) AS image
            ...
            $query = $this->db->query("SELECT DISTINCT d.acc_guid, UPPER(e.acc_name) as acc_name
              , IF(e.acc_guid = '13EE932D98EB11EAB05B000D3AA2838A', 'http://b2b.xbridge.my/asset/uploads/pandamart/pandamart.jpg' , CONCAT('http://b2b.xbridge.my/asset',file_path,'/',SUBSTRING_INDEX(file_path, '/', -1) , '.jpg') ) AS image
    ...
    public function customer_info($customer_guid)
    {
        ...
        $query = $this->db->query("SELECT acc_name,b2b_database,b2b_hub_database,file_path,
        rest_url,accounting_doc, '0' AS redirect,'b2b_summary' as old_db
        FROM " . $this->tb_lite_b2b . ".acc
    ...
    public function get_notification($customer_guid, $user_guid)
    {
        ...
        DATE_FORMAT(Registration_date, '%e-%M-%Y') AS reg_date,
        DATE_FORMAT(NOW(), '%e-%M-%Y') AS NOW
        FROM lite_b2b.`query_outstanding` a
        INNER JOIN lite_b2b.`set_supplier_user_relationship` b ON a.`customer_guid` = b.customer_guid
        AND a.`supplier_guid` = b.`supplier_guid`
    ...
    {
        $query = $this->db->query("SELECT * FROM " . $this->tb_lite_b2b . ".other_doc_setting WHERE customer_guid = '$customer_guid' AND isactive = 1 ORDER BY seq ASC");
    ...
    public function dashboard($db, $customer_guid, $location_in, $scode_in, $scode_where, $dashboard_num_days_data, $check_super_admin)
    {
        ...
        } else if ($customer_guid == '8D5B38E931FA11E79E7E33210BD612D3' || $customer_guid == '403810171FA711EA9BB8E4E7491C3E1E' || $customer_guid == '1F90F5EF90DF11EA818B000D3AA2CAA9') {
            $data = array(
    ...
    public function sub_dashboard_propose_po($db, $customer_guid, $location_in, $scode_in, $scode_where, $dashboard_num_days_data, $check_super_admin)
    {
        ...
        $proposed_po = $this->db->query("SELECT '' as count_doc,
             'proposed_po' as type,
             'Proposed Po' as display_name,
             '#931adf' AS background_color,
             'dashboard/DII.png' AS icon ");

        $proposed_po_supplier = $this->db->query("SELECT '' as count_doc,
              'proposed_po_supplier' as type,
              'Proposed Po Supplier' as display_name,
              '#931adf' AS background_color,
              'dashboard/DII.png' AS icon ");
        ...
        if ($select_type == 'data') {
          ...
          *
          ...
          ";
            ...
            echo 'select type not found';die;
    ...
    public function pci_list($db, $customer_guid, $location_in, $scode_in, $scode_where, $status_in, $refno, $period_code, $date_from, $date_to, $exp_from, $exp_to, $status_where, $limit, $offset, $select_type)
    {
        if ($refno == '') {
            ...
            $refno_where = "AND a.promo_refno LIKE '%" . $refno . "%'";
        }
    ...
    public function rb_list($db, $customer_guid, $location_in, $scode_in, $scode_where, $status_in, $refno, $period_code, $date_from, $date_to, $exp_from, $exp_to, $status_where, $limit, $offset, $select_type)
    {
        ...
        if ($select_type == 'data') {
            ...
            $select = "
            dbnote_guid,
            IFNULL(a.converted_by, '') AS prdn_refno,
            batch_no as refno,
            ...
            IF(STATUS = '-1', 'Rejected', 'Undefined'))))) AS status,
            IF(STATUS = '0', '#7fbfe1',
            IF(STATUS = '1', '#00c92b',
            IF(STATUS = '2', '#048ad1',
            IF(STATUS = '3', '#048ad1',
            IF(STATUS = '-1', '#ff4e25', '#048ad1'))))) AS color,
            STATUS,
            accepted_at,
            DATEDIFF(a.expiry_date,now()) as due_date
          ";
        ...
        WHERE a.customer_guid = '$customer_guid'
          ...
          ORDER BY batch_no ASC ,a.doc_date DESC
          $limit
           ;");
    ...
    public function ConsignSS_list($db, $customer_guid, $location_in, $scode_in, $scode_where, $status_in, $search_value, $period_code, $date_from, $date_to, $exp_from, $exp_to, $status_where, $limit, $offset, $select_type)
    {
        if ($search_value == '') {
            $refno_where = '';
        } else {
        ...
        FROM
           ...
           LEFT JOIN
             (SELECT *,
              ...
              GROUP BY LEFT(date_trans, 7),
                       supcus_code)b
            ON LEFT(a.date_trans, 7) = LEFT(b.date_trans, 7)
            AND a.supcus_code = b.supcus_code
    ...
    public function notification($customer_guid, $user_guid, $location_in, $list_where, $db, $redirect)
    {
        ...
        if ($redirect == '0') {
          ...
          $list_where");
            ...
            echo 'Redirect Version Incorrect';die;
    ...
    public function app_image_path()
    {
        ...
        $query = $this->db->query("SELECT `value` FROM " . $this->tb_lite_b2b_apps . ".config WHERE type = 'apps_image' ");
    ...
    public function download_path()
    {
        ...
        $query = $this->db->query("SELECT `value` FROM " . $this->tb_lite_b2b_apps . ".config WHERE type = 'download_path' ");
    ...
    public function b2b_invoice_list($con_deb_code, $select_type)
    {
        if ($select_type == 'data') {
            $select = '*';
        } else if ($select_type == 'counting') {
    ...
    public function monthly_invoice_list($status, $module_code, $supplier_guid, $select_type)
    {
        if ($status == "All") {
            $status_where = '';
        } else {
            $status_where = 'WHERE a.inv_status IN (' . $status . ')';
        }
        ...
        if (in_array('IAVA', $module_code)) {
            ...
            $query = $this->db->query("
                ...
                $status_where
                ORDER BY a.invoice_number DESC
                ");
            ...
            $select
            from " . $this->tb_b2b_invoice . ".supplier_monthly_main AS a
            WHERE biller_guid IN (" . $supplier_guid . ")
            AND inv_status != 'New'
            $status_where
            ORDER BY a.invoice_number DESC
            ");
    ...
    public function monthly_invoice_by_retailer_list($status, $module_code, $supplier_guid, $customer_guid, $select_type)
    {
        if ($status == "All") {
            $status_where = '';
        } else {
    ...
    public function module_group_list()
    {
        ...
        $query = $this->db->query("SELECT *
        FROM lite_b2b.set_module_group
        ORDER BY module_group_seq ASC");

        return $query;
    }

    public function user_module_list($module_group_guid)
    {

        $query = $this->db->query("SELECT a.*,b.`acc_module_group_name`
        FROM lite_b2b.acc_module a
        INNER JOIN lite_b2b.acc_module_group b
        ON a.`acc_module_group_guid` = b.`acc_module_group_guid`
        WHERE a.isenable = 1
        AND a.acc_module_group_guid = '$module_group_guid'");
